...
int getKth(int * nums1, int start1, int end1, int * nums2, int start2, int end2, int k)
{
    ...
    // 递归实现二分查找
    int min1 = MIN(len1, k/2);
    int min2 = MIN(len2, k/2);
    int compareIndex1 = start1 + min1 - 1;
    int compareIndex2 = start2 + min2 - 1;
    if (nums1[compareIndex1] < nums2[compareIndex2]) {
        return getKth(nums1, compareIndex1 + 1, end1, nums2, start2, end2, k - min1);
    }
    else {
        return getKth(nums1, start1, end1, nums2, compareIndex2 + 1, end2, k - min2);
    }
}
